...
import { isBefore } from "../../src/date/isBefore.js";
...
describe("isBefore", () => {
  ...
  it("returns false when the first date is after the second date", () => {
    const left = new Date(2026, 8, 25);
    const right = new Date(2026, 8, 20);

    expect(isBefore(left, right)).toBe(false);
  });
  ...
  it("returns false when the first time is later on the same day", () => {
  ...
  it("handles dates across different months", () => {
    const left = new Date(2026, 8, 30);
    const right = new Date(2026, 9, 1);

    expect(isBefore(left, right)).toBe(true);
  });

  it("handles dates across different years", () => {
    const left = new Date(2026, 11, 31);
    const right = new Date(2027, 0, 1);
  ...
  it("handles the Unix epoch", () => {
    ...
    expect(isBefore(epoch, later)).toBe(true);
  });

  it("does not mutate either date", () => {
    const left = new Date(2026, 8, 20, 10, 30);
    const right = new Date(2026, 8, 25, 15, 45);
